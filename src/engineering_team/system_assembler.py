...
class SystemAssembler:
    ...
    def assemble_system(
        self, architecture: SystemArchitecture, modules: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        integration_result = self._create_system_integration_with_retry(
    ...
    def _create_system_integration_with_retry(
        self,
        ...
        for attempt in range(max_retries + 1):
            ...
            if attempt == 0:
                integration_result = self._create_system_integration(
                    architecture, modules
                )
            else:
                integration_result = self._retry_integration_with_feedback(
                    architecture, modules, previous_result, validation_issues
                )

            # Validate the integration code quality
            validation_issues = self._validate_generated_code_quality(
                integration_result, "system_integration.py", architecture
            )

            if not validation_issues:
                print(
                    f"✅ System integration validation passed on attempt {attempt + 1}"
                )
                ...
                if attempt < max_retries:
                    ...
                    print(
                        f"❌ Integration validation failed after {max_retries + 1} attempts"
                    )

        return integration_result

    def _create_system_integration(
        self, architecture: SystemArchitecture, modules: Dict[str, Any]
    ) -> str:
        ...
        for module_spec in architecture.modules:
            ...
            if module_spec.dependencies:
                ...
                for dep in module_spec.dependencies:
                    dep_class = next(
                        (m.class_name for m in architecture.modules if m.name == dep),
                        None,
                    )
                    if dep_class:
    ...
    def {method}(self, *args, **kwargs):
        ...
        integration_class_name = (
            f"{architecture.system_name.replace(' ', '').replace('-', '')}Integration"
        )
    ...
    def _create_readme_documentation(
        self, architecture: SystemArchitecture, modules: Dict[str, Any]
    ) -> str:
        """Create comprehensive README documentation"""
            ...
            if file_path.exists():
                try:
                    ...
                    if len(content) < 50:  # File is too small to be meaningful
                        issues.append(
                            f"Generated file {file_name} appears to be empty or too small"
                        )
                except Exception as e:
    ...
    def _validate_generated_code_quality(
        self, code: str, filename: str, architecture: SystemArchitecture
    ) -> List[str]:
        ...
        if filename == "system_integration.py":
            ...
            if f"class {expected_class}" not in code:
                issues.append(
                    f"{filename} missing expected integration class: {expected_class}"
                )

        return issues

    def _retry_integration_with_feedback(
        self,
        architecture: SystemArchitecture,
        modules: Dict[str, Any],
        previous_result: str,
